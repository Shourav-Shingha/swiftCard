...
// js for show some product
const allProducts=()=>{fetch("https://fakestoreapi.com/products")
.then((res)=>res.json())
.then((data)=>cardContainer(data));

const cardContainer=(data)=>{
    const productDisplay=document.getElementById("productDisplay")
    productDisplay.innerHTML="";
    data.slice(0,3).forEach(datas => {
        const btnDiv=document.createElement("div");
        btnDiv.innerHTML=`<div class="card mx-auto bg-gray-100 rounded-2xl border border-gray-200 shadow-2xl ">
      <figure>
      ...
      <div class="card-body bg-white rounded-b-2xl">
        <div class="flex justify-between items-center">
          ...
          <p class="text-end "><i class="fa-solid fa-star fa-beat-fade"
              style="color: rgba(255, 212, 59, 1);"></i>${datas.rating.rate} (${datas.rating.count})</p>
        </div>
        ...
        <div class="card-actions justify-between gap-5">
          ...
          <button
            class="btn btn-xs sm:btn-sm md:btn-md lg:btn-lg xl:btn-xl bg-fuchsia-300 hover:bg-fuchsia-600 hover:text-white md:flex-1"><i
              class="fa-brands fa-opencart  hover:text-white"></i>Add</button>
        </div>
      </div>
    </div>`;
